...
//#define VOLTAGE_WARN_DELAY_TICKS 375 //when awake, a tick is 16ms. When sleeping, a tick is 0.5s. TODO: Seems to use both? A value opf 375 here means a delay of ~49s which doesn't fit either (~6s or ~187s respectively, comes out to ~7.6 ticks per second?)
#define VOLTAGE_WARN_DELAY_TICKS 32 //when awake, a tick is 16ms. When sleeping, a tick is 0.5s. TODO: Seems to use both? A value opf 375 here means a delay of ~49s which doesn't fit either (~6s or ~187s respectively, comes out to ~7.6 ticks per second?)
//#define VOLTAGE_WARN_INSTANT 294 //always warn instantly if voltage drops below this level (even with voltage drop)
